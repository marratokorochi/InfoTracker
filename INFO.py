...
def phoneGW():
    User_phone = input(
        f"\n {Wh}Enter phone number target {Gr}Ex [+880xxxxxxxxx] {Wh}: {Gr}")  # INPUT NUMBER PHONE
    default_region = "BD" 

    parsed_number = phonenumbers.parse(User_phone, default_region)  # VARIABLE PHONENUMBERS
    region_code = phonenumbers.region_code_for_number(parsed_number)
    jenis_provider = carrier.name_for_number(parsed_number, "en")
    location = geocoder.description_for_number(parsed_number, "BD")
...
------------------------------------------------------
        ...
        {Wh} "IN THE END! RED, WHITE AND BLUE TEAMS WILL CHECK TOGETHER HOW SECURITY CONTROLS CAN BE BREAK! BE READY FOR ONCE REAL THREAT."
    """)
...
def run_banner():
    ...
    stderr.writelines(f"""{Wh}
         .-.
       .'   `.          {Wh}---------------------------------------------------
       :O O   :         {Wh}|       {Gr}INFO - TRACKER - IP ADDRESS               {Wh}|
       : R    `.        {Wh}| Web: https://marratokorochi.github.io/oreoo/    {Wh}|
      :         ``.     {Wh}---------------------------------------------------
     :             `.
    :  :         .   `.
    :   :          ` . `.
